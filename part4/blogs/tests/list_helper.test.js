...
const { blogs, listWithOneBlog, tieBlogs, tieAuthors } = require('./test_helper')
...
describe('favorite blogs', () => {
  ...
  test('when multiple blogs have the same max likes, returns any of them', () => {
    ...
    assert.ok(possibleResults.some(b => b.title === result.title && b.author === result.author))
  })
})
...
describe('most blogs', () => {
  ...
  test('when multiple blogs have the same max likes, returns any of them', () => {
    ...
    assert.ok(possibleResults.some(b => b.author === result.author && b.blogs === result.blogs))
  })
})
...
describe('most likes', () => {
  ...
  test('when multiple blogs have the same max likes, returns any of them', () => {
    ...
    assert.ok(possibleResults.some(b => b.author === result.author && b.likes === result.likes))
  })
})