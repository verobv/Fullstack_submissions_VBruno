...
describe('when there are initially some blogs save', () => {
    ...
    beforeEach(async () => {
      ...
      const newUser = { username: 'testuser', password: 'testpass', name: 'Test User' }
    ...
    describe('When adding one new blog', () => {
        ...
        test('missing likes default to 0', async () => {
          ...
          const blogsAtEnd = await helper.blogsInDb()
          const addedBlog = blogsAtEnd.find(b => b.title === 'New Blog Post No Likes')
    ...
    describe('deletion of a blog', () => {
        test('succeeds with status code 204 if id is valid', async () => {
          ...
          const blog = { title: 'Blog to delete', author: 'Author', url: 'http://example.com' }
          await api
            .post('/api/blogs')
            .set('Authorization', `Bearer ${token}`)
            .send(blog)
          ...
          const blogToDelete = blogsAtStart.find(b => b.title === 'Blog to delete')
